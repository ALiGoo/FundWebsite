...
def test(request):
    if request.method == "POST":
        global mds_img
        profit_img, mds_img, profit_indicator = img(start=datetime.strptime("-".join([request.POST['start_year'], request.POST['start_month']]), '%Y-%m'),
...
def index(request):
    ...
    items = pd.read_sql(sql='select * from basic_information limit 20', con=engine)
